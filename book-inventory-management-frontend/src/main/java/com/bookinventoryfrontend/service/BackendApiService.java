 ...
 * BackendApiService
 *
 * All RestClient calls to the backend go here.
 * Controllers never talk to the backend directly — always via this service.
 *
 * HOW IT WORKS:
 * 1. Method makes a RestClient call to the backend
 * 2. Backend returns ApiResponse<T> wrapper (success/statusCode/message/data)
 * 3. We extract the "data" field and return it
 * 4. If backend returns error, we throw a RuntimeException with the message
 *
 * HOW JWT IS SENT:
...
public class BackendApiService {
     ...
     * Returns: LoginResponseDTO containing token + user info
     */
     ...
     * Returns: UserResponseDTO of newly created user
     */
     ...
     * Returns: Map with valid/userName/userId/roleName/message
     */
    ...
    public PermRoleResponseDTO getRoleById(Integer roleNumber) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/roles/{id}", roleNumber)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public Map<String, Object> getRoleUserCount(Integer roleNumber) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/roles/{id}/user-count", roleNumber)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
     ...
     * Returns: profile + purchase count + inventory IDs in one shot
     */
    public UserDashboardDTO getMyDashboard(String jwtToken) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/user/dashboard")
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public Long getMyPurchaseCount(String jwtToken) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/user/purchases/count")
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public Boolean checkMyPurchase(String jwtToken, Integer inventoryId) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/user/purchases/check/{id}", inventoryId)
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public List<PurchaseLogResponseDTO> getAllPurchases(String jwtToken) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/store-owner/purchases")
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public List<PurchaseLogResponseDTO> getPurchasesByUser(String jwtToken, Integer userId) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/store-owner/purchases/user/{id}", userId)
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public List<PurchaseLogResponseDTO> getPurchasesByInventory(String jwtToken, Integer inventoryId) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/store-owner/purchases/inventory/{id}", inventoryId)
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public Map<String, Object> getPurchaseStats(String jwtToken) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/store-owner/purchases/stats")
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    public List<UserResponseDTO> searchUsers(String jwtToken, String firstName, String lastName) {
        ...
        if (firstName != null && !firstName.isBlank()) uri.append("firstName=").append(firstName).append("&");
        if (lastName  != null && !lastName.isBlank())  uri.append("lastName=").append(lastName);
    ...
    public List<UserResponseDTO> getUsersByRole(String jwtToken, Integer roleNumber) {
        Map<String, Object> response = restClient.get()
                .uri("/api/v1/admin/users/by-role/{roleNumber}", roleNumber)
                .header("Authorization", "Bearer " + jwtToken)
                .retrieve()
                .body(new ParameterizedTypeReference<Map<String, Object>>() {});
    ...
    /**
     * Extracts the "data" field from the backend ApiResponse wrapper
     * and converts it to the given target class using Jackson.
     */
    @SuppressWarnings("unchecked")
    private <T> T extractData(Map<String, Object> response, Class<T> targetClass) {
        if (response == null) throw new RuntimeException("No response from backend");
    ...
    /**
     * Extracts "data" as a List of Maps (for endpoints returning List of custom objects)
     */
    @SuppressWarnings("unchecked")
